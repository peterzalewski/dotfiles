return {
   {
      ...
      keys = {
         ...
         { "<leader>gc", "<cmd>FzfLua grep_cword<cr>", desc = "Live grep for the current word" },
         {
            "<leader>gg",
            function()
               require("fzf-lua").live_grep_native({ rg_opts = "--hidden --glob=!.git/ --column --line-number --no-heading --color=always --smart-case --max-columns=4096 --max-columns-preview" })
            end,
            desc = "Live grep through the current project",
         },
